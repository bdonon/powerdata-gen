...
def sample_active_load(net, default_net, total_load, method, param):
    ...
    if method == 'homothetic':
        ...
        sample_uniform_independent_values(net, total_load, param)
    elif method == 'normal_independent_values':
        sample_normal_independent_values(net, total_load, param)
...
def sample_uniform_independent_factor(net, default_net, total_load, param):
    ...
    factor = sample_uniform_simplex(param, size=n_load, center_around_zero=True)
...
def sample_normal_independent_factor(net, default_net, total_load, param):
    ...
    n_load = len(net.load)
    default_value = default_net.load.p_mw / default_net.load.p_mw.sum()
    factor = sample_normal_simplex(param, size=n_load, center_around_zero=True)
    net.load.p_mw = (factor + default_value) * total_load


def sample_uniform_independent_values(net, total_load, param):
    """Samples independent loads uniformly, while respecting total_load."""
    n_load = len(net.load)
    factor = sample_uniform_simplex(param, size=n_load)
    net.load.p_mw = total_load * factor


def sample_normal_independent_values(net, total_load, param):
    """Samples independent loads normally, while respecting total_load."""
    n_load = len(net.load)
    factor = sample_normal_simplex(param, size=n_load)
    net.load.p_mw = total_load * factor
